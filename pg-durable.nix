...
buildPgrxExtension (finalAttrs: {
  ...
  #
  # ----------------------------------------------------------------------
  # Upgrading pg_durable (when moving this `version` to a new release)
  # ----------------------------------------------------------------------
  # We bundle Microsoft's pg_durable SQL skill inside the work-instruction
  # tool `wi-workflow/pg-durable-tool/vendor/pg-durable-sql-skill.md`, so the
  # docs our users read always match the extension we ship. That vendored
  # copy is downloaded from the SAME published tag as `version` above — never
  # from `main` — so the two never drift.
  #
  # When you bump `version` (and the resulting `src` hash and `cargoHash`):
  #
  #   1. Update `version` below.
  #   2. Let Nix resolve the new hashes:
  #        nix-build ... pg-durable.nix   # copy the hash Nix reports
  #      Set `sha256`/`cargoHash` from that output. Note `finalAttrs.version`
  #      flows into the `src.url` below, so the URL tracks the bump
  #      automatically.
  #   3. Replace the bundled skill from the matching tag:
  #        V=0.2.3   # <- the NEW version
  #        curl -fsSL https://raw.githubusercontent.com/microsoft/pg_durable/v${V}/.agents/skills/pg-durable-sql/SKILL.md \
  #          -o ../../wi-workflow/pg-durable-tool/vendor/pg-durable-sql-skill.md
  #      (The file's provenance banner at the top is regenerated by that
  #      full overwrite — do not hand-patch it.)
  #
  # The skill path in wi-workflow is pinned by name here. If that directory
  # or the vendored file moves, update this comment and the SKILL.md
  # cross-reference at the same time.
  # ----------------------------------------------------------------------
  src = fetchurl {
